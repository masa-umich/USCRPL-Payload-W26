...
// --- Replace your old flush variables with this ---
unsigned long lastFlushTime = 0;
const unsigned long FLUSH_INTERVAL = 5000; // Flush every 5000 milliseconds (5 seconds)

// BNO08x Data Caches (Stores the most recent reading)
...
void setup() {
  ...
  // 2. VERY IMPORTANT: Pull all CS pins HIGH immediately
  ...
  // --- THE FIX ---
  // Wait half a second to let all the external sensors fully power up.
  // This prevents the Teensy from outrunning the ADXL355's boot sequence.
  delay(5000); 
  // ---------------
  ...
  pinMode(SCH_CS, OUTPUT);
  digitalWrite(SCH_RESET, LOW); delay(100); digitalWrite(SCH_RESET, HIGH); delay(100);
  ...
  if (!SD.begin(SD_CS)) errorHalt("SD Card failed!");
  ...
  if (!dataFile) errorHalt("Could not create file!");
  ...
  // 5. Initialize ADXL355
  adxl355.beginSPI(ADXL_CS);
  adxl355.setRange(PL::ADXL355_Range::range2g);
  adxl355.enableMeasurement();
  
  auto testRead = adxl355.getAccelerations();
  if (testRead.x == 0 && testRead.y == 0 && testRead.z == 0) errorHalt("ADXL355 failed to start!");
  Serial.println("ADXL355 Ready.");

  // 6. Initialize BNO08x (using SPI)
  if (!bno08x.begin_SPI(BNO_CS, BNO_INT)) errorHalt("BNO08x failed to start!");
  
  // Set desired BNO reports (Requesting at 10000us / 100Hz intervals)
...
void loop() {
  ...
  // Changed from 'while' to 'if' to prevent infinite queue loops
  ...
  if (dataFile) {
    ...
    dataFile.print(adxl_res.x, 4); dataFile.print(",");
    dataFile.print(adxl_res.y, 4); dataFile.print(",");
    dataFile.print(adxl_res.z, 4); dataFile.print(",");
    ...
    // Only pause the Teensy to physically save to the card every 5 seconds.
    // The rest of the time, data is writing at lightning speed to the RAM buffer.
    if (millis() - lastFlushTime >= FLUSH_INTERVAL) {
      dataFile.flush();
      lastFlushTime = millis();
      
      // Optional: Flash the LED Blue briefly to indicate a physical save occurred
      leds[0] = CRGB::Blue; 
      FastLED.show();
    }
  }
  ...
  // --- 4. VISUAL HEARTBEAT ---
  // Keep the green pulsing heartbeat so you know the loop is flying
